    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                Cookies.set('jwtToken', token, { expires: 7 }); // Set the cookie to expire in 7 days

                // Registration successful, handle success scenario
                // For example, show a success message and redirect to the desired page
                router.push('/');
            } else {
                ...
                <div className="g-6 flex h-full flex-wrap items-center justify-center lg:justify-between">
                    ...
                    <div className="md:w-8/12 lg:ml-6 lg:w-5/12">
                        <form onSubmit={handleFormSubmit}>
                            ...
                            <div className="relative mb-6" data-te-input-wrapper-init>
                                <input
                                    type={toggleEye ? "text" : "password"} // Update the type attribute
                                    name='Password'
                                    value={password}
                                    onChange={handleOnChangeValue}
                                    className=" block min-h-[auto] w-full  rounded border-0 bg-white shadow-lg px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear font-semibold text-gray-500 "
                                    placeholder="Password" />
                                <div onClick={handleToggleEye} className="absolute inset-y-0  hover:bg-gray-200 rounded-full  right-0 px-3 py-1.5 flex items-center justify-center text-sm leading-5">
                                    {toggleEye ? <AiFillEyeInvisible size={18} className="text-gray-500" /> : <AiFillEye size={18} className="text-gray-500" />}
                                </div>
